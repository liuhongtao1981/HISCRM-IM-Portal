...
```json
{
  "common": {
    "intervalMin": 1,
    "intervalMax": 10,
    "description": "爬虫通用配置：最小和最大间隔时间（分钟）"
  }
}
```
...
| 参数 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `intervalMin` | number | 1 | 爬虫最小间隔（分钟） |
| `intervalMax` | number | 10 | 爬虫最大间隔（分钟） |

#### 2.2 评论爬虫 (`commentCrawler`)
