...
async function verifyDMExtraction() {
  ...
  try {
    ...
    console.log(`有二维码: ${loginStatus.hasQRCode ? '⚠️  �?(需要登�?' : '�?�?}`);
    console.log(`有登录按�? ${loginStatus.hasLoginButton ? '⚠️  �?(需要登�?' : '�?�?}`);
    ...
    if (allConversations.length === 0) {
      ...
      console.log('提示: 请检查页面是否加载完�?或选择器是否正确\n');
      return;
    }
    ...
    console.log('点击前状�?');
    console.log(`  �?[class*="message"]: ${beforeState.hasMessageClass ? '�? : '�?}`);
    console.log(`  �?[class*="chat"]: ${beforeState.hasChatClass ? '�? : '�?}`);
    console.log(`  URL 包含 /chat/: ${beforeState.urlIncludesChat ? '�? : '�?}`);
    console.log(`  �?textarea: ${beforeState.hasTextarea ? '�? : '�?}`);
    console.log(`  �?contenteditable: ${beforeState.hasContentEditable ? '�? : '�?}\n`);

    // 6. 检查原始验证逻辑
    const originalCheck = beforeState.hasMessageClass || beforeState.hasChatClass || beforeState.urlIncludesChat;
    console.log(`原始验证逻辑 (message || chat || url): ${originalCheck ? '�?会返�?true' : '�?会返�?false'}`);

    if (originalCheck) {
      console.log('⚠️  警告: 原始验证逻辑在点击前就返�?true!');
      console.log('   这意味着验证逻辑可能过于宽松\n');
    } else {
      console.log('�?原始验证逻辑正确: 点击前返�?false\n');
    }

    // 7. 点击第一个会�?
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    console.log('👆 第二�? 点击第一个会�?);
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n');
    ...
    console.log('点击后状�?');
    console.log(`  �?[class*="message"]: ${afterState.hasMessageClass ? '�? : '�?}`);
    console.log(`  �?[class*="chat"]: ${afterState.hasChatClass ? '�? : '�?}`);
    console.log(`  URL 包含 /chat/: ${afterState.urlIncludesChat ? '�? : '�?}`);
    console.log(`  �?textarea: ${afterState.hasTextarea ? '�? : '�?}`);
    console.log(`  �?contenteditable: ${afterState.hasContentEditable ? '�? : '�?}`);
    ...
    console.log(`原始验证逻辑 (message || chat || url): ${originalCheckAfter ? '�?会返�?true' : '�?会返�?false'}\n`);

    if (originalCheckAfter) {
      console.log('�?原始验证逻辑正确: 点击后返�?true');
    } else {
    ...
    console.log('\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    console.log('🔬 第三�? 测试消息提取 (React Fiber)');
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n');
    ...
    console.log('\n4. 建议:');
    if (originalCheck) {
      console.log('   - 建议使用更严格的验证逻辑 (检�?textarea �?contenteditable)');
      console.log('   - 当前验证逻辑可能在点击前就返�?true');
    } else {
      console.log('   - 原始验证逻辑工作正常,可以保留');
    }

    if (messages.length === 0) {
      console.log('   - 需要调�?React Fiber 消息提取逻辑');
      console.log('   - 可能需要更�?Fiber 遍历深度或数据结构匹�?);
    }
