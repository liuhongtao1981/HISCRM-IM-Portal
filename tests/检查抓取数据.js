...
dmByAccount.forEach((row, i) => {
  ...
  console.log(`    消息�? ${row.message_count}`);
  console.log(`    最后抓�? ${row.last_crawl}`);
});
...
recentDMs.forEach((dm, i) => {
  ...
  const contentPreview = dm.content ? dm.content.substring(0, 80) : '(无内�?';
  console.log(`    内容: ${contentPreview}${dm.content && dm.content.length > 80 ? '...' : ''}`);
  console.log(`    检测时�? ${new Date(dm.detected_at * 1000).toLocaleString('zh-CN')}`);
...
conversations.forEach((conv, i) => {
  const lastMsg = conv.last_message_content ? conv.last_message_content.substring(0, 50) : '(�?';
  console.log(`[${i+1}] ${conv.platform_user_name || '(未知用户)'}`);
  console.log(`    最后消�? ${lastMsg}${conv.last_message_content && conv.last_message_content.length > 50 ? '...' : ''}`);
...
console.log(`有ID的消�? ${withId.count}`);
console.log(`无ID的消�? ${withoutId.count}`);
