/**
 * 验证 replyToId 修复效果
 * 检查 Master 返回的数据中 replyToId 是否已正确转换为 null
 */
...
socket.on('monitor:topics', (data) => {
  ...
  if (topicsWithMessages.length > 0) {
    // 测试第一个有消息的主题
    const testTopic = topicsWithMessages[0];
...
socket.on('monitor:messages', (data) => {
  ...
  data.messages.forEach((msg, index) => {
    ...
    console.log(`  消息分类: ${msg.messageCategory || '(无)'}`);
  ...
  if (mainMessages.length === 0 && data.messages.length > 0) {
    console.log('\n❌ 警告：所有消息都被过滤掉了！');
    console.log('这意味着所有消息的 replyToId 都是 truthy 值（如 "0"）');
