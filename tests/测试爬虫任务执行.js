...
if (workers.length === 0) {
  ...
  workers.forEach((worker, index) => {
    ...
    console.log(`    分配账户�? ${worker.assigned_accounts}`);
    console.log(`    最后心�? ${new Date(worker.last_heartbeat * 1000).toLocaleString('zh-CN')}`);
...
if (monitorTasks.length === 0) {
  ...
  console.log('原因：Worker需要在登录成功后创建监控任�?);
} else {
  monitorTasks.forEach((task, index) => {
    console.log(`\n[${index + 1}] 任务 ID: ${task.id}`);
    console.log(`    账户 ID: ${task.account_id}`);
    console.log(`    任务类型: ${task.task_type}`);
    console.log(`    状�? ${task.status}`);
    console.log(`    间隔: ${task.interval_seconds} 秒`);
    console.log(`    最后运�? ${task.last_run_time ? new Date(task.last_run_time * 1000).toLocaleString('zh-CN') : '(从未运行)'}`);
    console.log(`    下次运行: ${task.next_run_time ? new Date(task.next_run_time * 1000).toLocaleString('zh-CN') : '(未计�?'}`);
...
if (accountsWithoutUserId.length > 0) {
  issues.push({
    level: 'ERROR',
    message: `${accountsWithoutUserId.length} 个账户缺�?platform_user_id`,
    solution: '需要重新登录以获取 platform_user_id'
  });
}

// �?核心问题：账户有 platform_user_id，但 Worker 仍报�?
...
if (accountsWithIdButError.length > 0) {
  accountsWithIdButError.forEach(acc => {
    issues.push({
      level: 'CRITICAL',
      message: `账户 ${acc.id} 数据库中�?platform_user_id (${acc.platform_user_id})，但 Worker 仍报错缺失`,
      solution: '需要重�?Worker 以重新加载账户配置，或者实现配置热更新机制'
    });
  });
}
...
if (accountsWithOtherErrors.length > 0) {
  accountsWithOtherErrors.forEach(acc => {
    issues.push({
      level: 'ERROR',
      message: `账户 ${acc.id} 有错�? ${acc.last_error_message}`,
      solution: '检�?Worker 日志，解决错误后重启 Worker'
    });
  });
}
...
if (monitorTasks.length === 0) {
  issues.push({
    level: 'WARNING',
    message: '没有创建任何监控任务',
    solution: '检查账户是否已分配�?Worker，Worker 是否正常启动'
  });
}
...
if (comments.length === 0 && messages.length === 0) {
  issues.push({
    level: 'INFO',
    message: '还没有收集到任何评论或私信数�?,
    solution: '等待爬虫运行，或检查账户是否有新数�?
  });
}
