/**
 * 分析抖音私信消息虚拟列表 - 使用全新浏览器目录
 *
 * 目标：
 * 1. 打开浏览器并导航到私信页面
 * 2. 等待30秒让用户手动点击一个会话
 * 3. 分析右侧消息列表的虚拟列表结构
 * 4. 深度搜索 React Fiber 树找到消息数据的实际属性名
 */
...
async function analyzeMessageVirtualList() {
  ...
  try {
    ...
    console.log('  1. 如果需要，请登录抖音创作者平台');
    console.log('  2. 点击左侧的一个会话');
    console.log('  3. 确保右侧显示了消息列表');
    console.log('  4. 可以滚动一下消息列表加载更多消息');
    console.log('='.repeat(80));
    console.log('');
    ...
    const allContainers = await page.evaluate(() => {
      ...
      grids.forEach((grid, index) => {
        ...
        containers.push({
          ...
          innerClassName: innerContainer ? innerContainer.className : '(无)',
          // 获取第一个子元素的文本预览来判断内容
    ...
    // 步骤 2: 深度分析每个容器
    console.log('='.repeat(80));
    console.log('步骤 2: 深度分析每个容器的 React Fiber 结构');
    console.log('='.repeat(80) + '\n');
    ...
    for (let containerIdx = 0; containerIdx < allContainers.length; containerIdx++) {
      ...
      const analysis = await page.evaluate((idx) => {
        ...
        // 深度搜索 React Fiber 树
        function deepSearchFiber(fiber, depth = 0, maxDepth = 15, path = '') {
          ...
          if (fiber.memoizedProps) {
            ...
            // 查找所有可能的消息相关属性
            const messageRelatedKeys = allKeys.filter(key => {
            ...
            if (messageRelatedKeys.length > 0) {
              ...
              findings.push({
                ...
                messageRelatedKeys,
                sampleData,
                allPropsKeys: allKeys.slice(0, 20) // 只保存前20个键名
              });
            }
          }
      ...
      console.log(`  已分析: 前 ${analysis.analyzedCount} 个`);
      console.log(`  找到包含消息相关数据的节点: ${analysis.findings.length}\n`);
      ...
      if (analysis.findings.length > 0) {
        ...
        analysis.findings.slice(0, 5).forEach((finding, idx) => {
          ...
          console.log(`    总Props数: ${finding.totalPropsCount}`);
          console.log(`    所有Props键 (前20个): ${finding.allPropsKeys.join(', ')}`);
          console.log(`    消息相关键 (${finding.messageRelatedKeys.length}个): ${finding.messageRelatedKeys.join(', ')}`);
          console.log(`    数据样本:`);
