/**
 * 验证脚本: 检查私信会话的时间戳显示是否正确
 *
 * 目的: 诊断客户端截图中显示的日期(11/01, 10/28, 01/21)来源
 */
...
conversations.forEach((conv, index) => {
  ...
  console.log(`   lastMessageTime 时间戳: ${lastMessageTime}`);
  ...
  if (lastMessageTime) {
    ...
    console.log(`   格式: ${isMilliseconds ? '毫秒级 (13位)' : '秒级 (10位)'}`);
...
console.log('═══════════════════════════════════════════════════════\n');
console.log('💡 如何验证:');
console.log('   1. 对比客户端截图中显示的日期');
console.log('   2. 检查 "客户端应显示" 是否匹配截图');
