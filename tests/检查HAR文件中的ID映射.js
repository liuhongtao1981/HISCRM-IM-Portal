...
console.log(`找到 ${worksAPIs.length} 次作品列�?API 调用\n`);
...
if (worksAPIs.length > 0) {
  ...
  if (responseText) {
    ...
    if (respData.item_list && respData.item_list.length > 0) {
      ...
      // 打印所有关键字�?
      console.log(`标题: ${firstItem.desc || firstItem.title || '�?}`);
      console.log(`\nID 相关字段:`);
      ...
      console.log(`\n其他关键字段:`);
      console.log(`  评论�? ${firstItem.statistics?.comment_count || 0}`);
      console.log(`  创建时间: ${firstItem.create_time || '�?}`);
      ...
      console.log(`\n📊 统计结果 (�?${respData.item_list.length} 个作�?:`);
      console.log(`  �?aweme_id: ${hasAwemeId} 个`);
      console.log(`  �?sec_item_id: ${hasSecItemId} 个`);
      console.log(`  两种 ID 都有: ${hasBothIds} 个`);

      if (hasBothIds === respData.item_list.length) {
        console.log(`\n�?所有作品都包含两种 ID！可以建立映射关系`);
      } else {
        console.log(`\n⚠️  不是所有作品都有两�?ID，需要进一步检查`);
      }
    }
  }
}
...
console.log(`找到 ${commentAPIs.length} 次评论列�?API 调用\n`);
...
if (commentAPIs.length > 0) {
  ...
  if (responseText) {
    ...
    if (respData.comments && respData.comments.length > 0) {
      ...
      console.log(`\n其他关键字段:`);
      console.log(`  作�? ${firstComment.user?.nickname || '�?}`);
      console.log(`  创建时间: ${firstComment.create_time || '�?}`);
      ...
      console.log(`\n📊 统计结果 (�?${respData.comments.length} 条评�?:`);
      console.log(`  �?aweme_id: ${hasAwemeId} 条`);
      console.log(`  �?sec_aweme_id: ${hasSecAwemeId} 条`);
...
console.log('\n\n💡 结论和建�?');
console.log('='.repeat(80));
console.log('基于以上分析，我们需�?');
console.log('1. 在爬取作品时，同时保�?aweme_id �?sec_item_id');
console.log('2. 在爬取评论时，检查是否有 sec_aweme_id 字段');
console.log('3. 使用 aweme_id 作为关联的主�?);
