...
async function testBackButtonSelector() {
  ...
  console.log(`📂 浏览器数据目�? ${userDataDir}\n`);
  ...
  if (conversations > 0) {
    ...
    try {
      ...
      if (await elem.isVisible()) {
        ...
        if (listCount > 0) {
          console.log(`�?成功返回到会话列�?(${listCount} 个会�?`);
        } else {
          console.log('�?点击后未返回到列�?);
        }
      } else {
        console.log('�?.semi-button-content 不可�?);
      }
    } catch (error) {
