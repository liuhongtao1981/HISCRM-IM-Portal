...
async function extractDiscussionsFromDOM() {
  ...
  try {
    ...
    // 5. 点击按钮
    console.log(`🖱�? 点击 ${buttonInfo.text}...\n`);
    ...
    console.log(`提取�?${preciseDiscussions.length} 条精确的讨论数据:\n`);
    ...
    if (preciseDiscussions.length > 0) {
      console.log('�?成功从DOM提取到讨论数�?');
      console.log('   这证明讨论数据在DOM�?而不是通过单独的API加载\n');
      console.log('💡 建议: 修改爬虫从DOM直接提取讨论数据\n');
    } else {
      console.log('⚠️  未能从DOM提取到讨论数据\n');
      console.log('可能原因:');
      console.log('  1. 选择器不正确');
      console.log('  2. 讨论数据结构与预期不�?);
