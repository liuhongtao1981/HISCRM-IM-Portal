...
async function testCommentDiscussionFlow() {
      ...
      if (/comment.*reply/i.test(url)) {
        ...
        if (json.reply_list && Array.isArray(json.reply_list)) {
          interceptedAPIs.discussions.push({
            ...
            data: json.reply_list[0], // 保存第一条用于分�?
          });
  ...
  const videos = await page.evaluate(() => {
    ...
    return results.sort((a, b) => b.commentCount - a.commentCount); // 评论多的排前�?
  });

  console.log(`📊 找到 ${videos.length} 个有评论的视�?`);
  videos.slice(0, 3).forEach((v, i) => {
    console.log(`  ${i + 1}. ${v.title.substring(0, 40)}... (${v.commentCount} 条评�?`);
  });
  console.log('');
  ...
  console.log(`🎯 选择视频: ${targetVideo.title.substring(0, 50)}... (${targetVideo.commentCount} 条评�?\n`);
  ...
  while (scrollAttempts < maxScrolls) {
    ...
    if (currentCommentCount > lastCommentCount) {
      console.log(`  📊 当前已加�?${currentCommentCount} 条评论`);
      lastCommentCount = currentCommentCount;
      await page.waitForTimeout(2000);
      scrollAttempts++;
    } else {
      console.log(`  �?评论加载完成 (�?${currentCommentCount} �?\n`);
      break;
    }
  }

  // 9. 查找并点击所�?"查看X条回�? 按钮
  console.log('🔍 查找所�?"查看X条回�? 按钮...\n');
  ...
  console.log(`📊 找到 ${replyButtonsInfo.length} 个回复按�?`);
  replyButtonsInfo.forEach((btn, i) => {
    console.log(`  ${i + 1}. ${btn.text} (${btn.replyCount} �?`);
  });
  console.log('');
  ...
  interceptedAPIs.comments.forEach((api, i) => {
    console.log(`  ${i + 1}. ${api.count} 条评�?(has_more: ${api.has_more})`);
  });
  ...
  if (interceptedAPIs.discussions.length > 0) {
    ...
    console.log('🔑 关键发现:');
    console.log('  - 讨论API只在点击"查看X条回�?后触�?);
