/**
 * 验证 Phase 3 修复 - 检查数据库中 conversation 的时间戳
 */
...
console.log('\n╔═══════════════════════════════════════════════════════╗');
console.log('║  Phase 3 修复验证 - 会话时间戳检查                    ║');
console.log('╚═══════════════════════════════════════════════════════╝\n');
...
if (conversations.length === 0) {
  ...
  console.log('   提示: Worker 可能还在抓取数据中');
  process.exit(0);
}
...
conversations.forEach((conv, index) => {
  ...
  console.log(`   格式: ${isMilliseconds ? '毫秒级 (13位)' : '❌ 格式错误!'}`);
  console.log('');
});
...
if (allValid) {
  console.log('🎉 所有会话时间戳都是毫秒级 (13位)!');
  console.log('');
  console.log('修复成功:');
  console.log('  ✅ Worker: normalizeTimestamp() 统一为毫秒级');
  console.log('  ✅ CacheDAO: 保持毫秒级不变');
  console.log('  ✅ Database: 存储毫秒级');
  console.log('  ✅ IM WebSocket: 应该使用 conversation.lastMessageTime');
  console.log('');
  console.log('IM 客户端应该正确显示 2025年11月3日 的时间');
} else {
