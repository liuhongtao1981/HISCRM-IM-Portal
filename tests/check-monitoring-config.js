...
if (account) {
  ...
  console.log(`  Worker: ${account.assigned_worker_id || '未分配'}`);
  console.log(`  手动分配: ${account.is_manually_assigned === 1 ? '是' : '否'}`);
} else {
  console.log('❌ 账户不存在');
}
...
if (workerConfig) {
  ...
  console.log('❌ Worker 配置不存在');
}

console.log('\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n');

// 检查 Worker 运行时状态
const workerRuntime = db.prepare('SELECT * FROM worker_runtime WHERE worker_id = ?').get('worker1');
if (workerRuntime) {
  console.log('📈 Worker 运行时状态：');
  console.log(`  状态: ${workerRuntime.status}`);
  console.log(`  PID: ${workerRuntime.process_id || '无'}`);
  const lastHeartbeat = workerRuntime.last_heartbeat_time ? new Date(workerRuntime.last_heartbeat_time).toLocaleString('zh-CN') : '无';
