/**
 * 直接读取 Master DataStore 的评论数�? * 检查每条评论的详细信息
 */
...
console.log('================================================================================');
console.log('🔍 读取 Master DataStore 的评论数�?);
console.log('================================================================================\n');
...
socket.on('dataStoreSnapshot', (data) => {
  ...
  console.log(`评论�? ${comments?.length || 0}`);
  console.log(`作品�? ${contents?.length || 0}`);
  console.log(`私信�? ${messages?.length || 0}`);
  console.log(`会话�? ${conversations?.length || 0}\n`);
  ...
  console.log(`涉及作品�? ${Object.keys(groupedByContent).length}\n`);
  ...
  Object.entries(groupedByContent).forEach(([contentId, commentList], idx) => {
    ...
    console.log(`  评论�? ${commentList.length}`);
    console.log('');
...
setTimeout(() => {
  console.error('�?超时 - /admin 命名空间可能不支�?getDataStoreSnapshot 事件');
  process.exit(1);
}, 10000);
