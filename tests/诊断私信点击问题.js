...
async function diagnoseDirectMessageClick() {
  ...
  try {
    ...
    Object.entries(selectorTests).forEach(([key, result]) => {
      console.log(`  ${key}:`);
      console.log(`    选择�? ${result.selector}`);
    ...
    console.log('📸 已保存截�? logs/dm-diagnosis-step1-list.png\n');

    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    console.log('📍 步骤3: 尝试点击第一个会�?);
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n');
    ...
    if (maxCount === 0) {
      console.log('�?未找到任何会话元�?');
      console.log('\n⚠️ 可能原因:');
      console.log('  1. 页面结构不同');
      console.log('  2. 需要点�?全部"标签');
      console.log('  3. 会话列表未加�?);
      ...
      if (allTabClicked) {
        ...
        console.log('�?未找�?全部"标签');
      }

    } else {
      console.log(`�?找到 ${maxCount} 个会话元�?(使用选择�? ${bestSelector})\n`);
      ...
      if (clickResult.success) {
        ...
        if (isOpen) {
          ...
          console.log('📸 已保存截�? logs/dm-diagnosis-step2-detail.png\n');
          ...
          console.log('⚠️ 当前验证逻辑可能不准�?需要更新选择器\n');

          // 截图保存失败状�?
          await page.screenshot({ path: './logs/dm-diagnosis-step2-failed.png', fullPage: true });
          console.log('📸 已保存截�? logs/dm-diagnosis-step2-failed.png\n');
        }
    ...
    console.log('关键发现:');
    console.log('  1. 最佳选择�?', bestSelector || '未找�?);
    ...
    console.log('建议修复:');
    if (maxCount === 0) {
      console.log('  - 检�?全部"标签是否需要点�?);
      console.log('  - 检查页面加载是否完�?);
      console.log('  - 检查选择器是否正�?);
    } else if (bestSelector !== '[role="list-item"]') {
      console.log(`  - 更新选择器从 [role="list-item"] �?${bestSelector}`);
    }
    ...
    try {
      ...
      console.log('📸 已保存错误截�? logs/dm-diagnosis-error.png');
    } catch (e) {
      // 忽略截图错误
    }
  } finally {
