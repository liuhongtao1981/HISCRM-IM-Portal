/**
 * 模拟用户快速多次点击同一个账户
 * 检查服务器返回的 topics 是否一致
 */
const io = require('socket.io-client');

const MASTER_URL = 'http://localhost:3000';

console.log('=== 模拟快速点击账户 ===\n');
...
socket.on('monitor:channels', (data) => {
  ...
  console.log('模拟用户快速点击 5 次，间隔 100ms...\n');
...
function analyzeResponses() {
  ...
  // 检查未读数是否一致
  const uniqueComments = [...new Set(responses.map(r => r.commentUnread))];
  const uniquePrivates = [...new Set(responses.map(r => r.privateUnread))];

  console.log(`评论未读数的唯一值: ${JSON.stringify(uniqueComments)}`);
  console.log(`私信未读数的唯一值: ${JSON.stringify(uniquePrivates)}`);

  if (uniqueComments.length > 1) {
    console.log(`\n⚠️  评论未读数在跳动！有 ${uniqueComments.length} 个不同的值`);
    console.log(`详细序列: ${responses.map(r => r.commentUnread).join(' → ')}`);
  } else {
    console.log(`\n✅ 评论未读数稳定 (${uniqueComments[0]})`);
  }
