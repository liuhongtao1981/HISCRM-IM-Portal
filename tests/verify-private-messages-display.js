...
socket.on('monitor:topics', (data) => {
  ...
  // 找messageCount > 0 的主�?  const topicsWithMsgs = privateTopics.filter(t => t.messageCount > 0);
  console.log(`有消息的私信 topics: ${topicsWithMsgs.length}`);
  console.log('');

  if (topicsWithMsgs.length === 0) {
    console.log('⚠️  所有私信主题都显示 messageCount=0');
    console.log('   这可能是 getTopicsFromDataStore 计算 messageCount 的问�?);
    console.log('');
    console.log('尝试直接请求�?个私信主题的消息...');
    console.log('');
      ...
      socket.once('monitor:messages', (msgData) => {
        if (msgData.topicId === topic.id) {
          ...
          if (completed === testTopics.length) {
            ...
            console.log('='.repeat(80));
            console.log('');
            console.log('如果�?topic 的实际消息数 > 0，但 messageCount=0�?);
            console.log('说明问题�?getTopicsFromDataStore �?messageCount 计算逻辑�?);
            console.log('');
            console.log('需要检�?im-websocket-server.js �?445 行附近的代码�?);
            console.log('');
            socket.disconnect();
            process.exit(0);
          }
    ...
    // 请求第一个有消息的主�?    const testTopic = topicsWithMsgs[0];
    ...
    socket.once('monitor:messages', (msgData) => {
      ...
      if (msgData.messages.length > 0) {
        ...
        console.log(`messageCategory='private' 的消�? ${privateMessages.length}`);
        console.log('');
