 ...
 * 修复登录状�?
 *
 * 修复�?Worker 误覆盖导致的 login_status 错误
 *
 * 逻辑�?
 * - 如果账户�?platform_user_id（说明登录过�?
 * - �?login_status = 'not_logged_in'（状态错误）
 * - 则修复为 'logged_in'
 *
 * 使用方法�?
 * node tests/修复登录状�?js
...
// 查找需要修复的账户：有 platform_user_id �?login_status �?not_logged_in
...
needFix.forEach((acc, index) => {
  ...
  console.log(`    当前登录状�? ${acc.login_status} ❌`);
  console.log(`    最后登录时�? ${acc.last_login_time ? new Date(acc.last_login_time * 1000).toLocaleString('zh-CN') : '(�?'}`);
...
try {
  ...
  if (fixed > 0) {
    ...
    console.log('');
    console.log('💡 下一步操�?');
    console.log('   1. 重启 Worker 以加载修复后的配�?);
    console.log('   2. 观察 Worker 日志，确认不再报�?);
    console.log('   3. 等待爬虫任务执行，验证数据采�?);
  } else {
    console.log('');
    console.log('⚠️  没有账户被修复，请检查数据库状�?);
  }

} catch (error) {
  console.error('�?修复过程中出�?', error);
  process.exit(1);
} finally {
  db.close();
}
