 ...
 * 触发私信爬虫测试懒加�?
 *
 * 通过 API 手动触发一次私信爬虫，验证 DataManager 懒加�?
 */

const http = require('http');

console.log('══════════════════════════════════════════════════════�?);
console.log('  触发私信爬虫测试懒加�?);
console.log('═══════════════════════════════════════════════════════\n');

const accountId = 'acc-98296c87-2e42-447a-9d8b-8be008ddb6e4';

console.log(`📋 目标账户: ${accountId}`);
console.log(`🎯 目标: 触发私信爬虫，验�?DataManager 懒加载\n`);
...
const req = http.request(options, (res) => {
  ...
  res.on('end', () => {
    console.log(`📥 响应状�? ${res.statusCode}`);
    ...
    if (res.statusCode === 200 || res.statusCode === 201) {
      ...
      console.log('  3. 检查是否出�?"Auto-creating DataManager" 日志\n');
    } else {
      console.log('�?触发失败，请检查：');
      console.log('  1. Master 服务器是否运行在端口 3000');
      console.log('  2. Worker 是否已连�?);
...
req.on('error', (error) => {
  ...
  console.error('\n请确保：');
  console.error('  1. Master 服务器正在运�?);
  console.error('  2. Master 监听�?localhost:3000\n');
  console.log('══════════════════════════════════════════════════════�?);
  process.exit(1);
});
