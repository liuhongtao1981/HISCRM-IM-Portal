...
async function main() {
  ...
  try {
    ...
    if (messages.length === 0) {
      ...
      console.log('');
      console.log('可能原因:');
      console.log('  1. Worker 还没有开始爬取数�?);
      console.log('  2. Master 内存数据还没有加�?);
      console.log('  3. API 路径或参数错�?);
      return;
    }
    ...
    messages.slice(0, 5).forEach((msg, idx) => {
      ...
      if (msg.data && msg.data.rawData) {
        ...
        console.log('    - secSender:', msg.data.rawData.secSender ? '�?' + msg.data.rawData.secSender.substring(0, 50) + '...' : '�?�?);
        console.log('    - secReceiver:', msg.data.rawData.secReceiver ? '�?' + msg.data.rawData.secReceiver.substring(0, 50) + '...' : '�?�?);
        console.log('    - nickname:', msg.data.rawData.nickname || '�?�?);
        console.log('    - �?20 个字�?', rawKeys.slice(0, 20).join(', '));
      } else {
        console.log('    ⚠️ 没有 rawData 对象');
        console.log('    data 字段包含:', msg.data ? Object.keys(msg.data).join(', ') : '�?data 字段');
      }
      console.log('');
    });
    ...
    messages.forEach(msg => {
      ...
      // 统计 rawData 字段�?      if (msg.data?.rawData) {
    ...
    console.log('  sender_id �?unknown:', unknownSenderId, '/', messages.length, '�?, `(${(unknownSenderId / messages.length * 100).toFixed(1)}%)`);
    console.log('');
    ...
    if (hasSecSender === messages.length && hasEncryptedConvId === messages.length && unknownSenderId === 0) {
      ...
      console.log('  �?0% 消息�?sender_id �?unknown');
      console.log('');
      ...
      if (unknownSenderId > 0) {
        console.log(`  �?${(unknownSenderId / messages.length * 100).toFixed(1)}% 消息�?sender_id �?unknown`);
      }
      console.log('');
      console.log('💡 说明: Worker 可能还在使用旧代码，需要重�?Worker');
    }
