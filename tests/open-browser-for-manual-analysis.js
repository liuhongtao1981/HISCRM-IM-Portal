/**
 * 打开浏览器供手动登录和分�? *
 * 使用步骤�? * 1. 脚本打开浏览�? * 2. 用户扫码登录抖音
 * 3. 用户点击一个会�? * 4. 在终端按 Enter 开始分�? * 5. 脚本自动分析虚拟列表并输出结�? */
...
async function openBrowserForAnalysis() {
  console.log('\n' + '='.repeat(80));
  console.log('打开浏览器供手动登录和分�?);
  console.log('='.repeat(80) + '\n');

  // 使用全新的测试目�?  const userDataDir = path.join(__dirname, '../test-browser-data-manual');
  ...
  try {
    ...
    console.log('='.repeat(80));
    console.log('请完成以下操作：');
    console.log('  1. 如果需要，扫码登录抖音创作者平�?);
    console.log('  2. 点击左侧的一个会�?);
    console.log('  3. 确保右侧显示了消息列�?);
    console.log('='.repeat(80) + '\n');

    await question('操作完成后，�?Enter 键开始分�?..');
...
openBrowserForAnalysis().catch(err => {
