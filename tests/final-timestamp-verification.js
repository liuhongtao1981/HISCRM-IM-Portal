...
// 1. 验证 cache_messages �?console.log('�?】cache_messages 表验�?\n');
...
console.log(`秒级时间�?(10�?: ${messageStats.seconds_count} 条`);
console.log(`毫秒级时间戳 (13�?: ${messageStats.milliseconds_count} 条`);
...
if (messageStats.seconds_count === 0 && messageStats.milliseconds_count === messageStats.total) {
  console.log('�?cache_messages 表：所有时间戳都是毫秒�?(13�?');
} else {
  console.log('�?cache_messages 表：存在格式不一致的时间�?);
}

console.log('\n');

// 2. 验证 cache_conversations �?console.log('�?】cache_conversations 表验�?\n');
...
console.log(`秒级时间�?(10�?: ${conversationStats.seconds_count} 个`);
console.log(`毫秒级时间戳 (13�?: ${conversationStats.milliseconds_count} 个`);
console.log(`超大时间�?(>13�?: ${conversationStats.invalid_count} 个`);
...
if (conversationStats.seconds_count === 0 && conversationStats.invalid_count === 0 && conversationStats.milliseconds_count === conversationStats.total) {
  console.log('�?cache_conversations 表：所有时间戳都是毫秒�?(13�?');
} else {
  console.log('�?cache_conversations 表：存在格式不一致的时间�?);
}

console.log('\n');

// 3. 验证消息和会话的时间戳一致�?console.log('�?】消息和会话的时间戳一致性验�?\n');
...
consistencyCheck.forEach((conv, index) => {
  ...
  if (conv.msg_last_time) {
    console.log(`   消息表最后时�? ${conv.msg_last_time} (${new Date(conv.msg_last_time).toLocaleString('zh-CN')})`);
    console.log(`   是否一�? ${isConsistent ? '�? : '�?}`);
  } else {
    console.log(`   消息表最后时�? 无消息`);
  }
...
if (allValid) {
  console.log('🎉 所有验证通过�?);
  console.log('');
  console.log('�?cache_messages: 所有时间戳都是毫秒�?);
  console.log('�?cache_conversations: 所有时间戳都是毫秒�?);
  console.log('�?消息和会话的时间戳保持一�?);
  console.log('');
  console.log('数据流程�?);
  console.log('  1. Worker 抓取 �?normalizeTimestamp() �?毫秒�?�?);
  console.log('  2. 数据库存�?�?INTEGER (毫秒�? �?);
  console.log('  3. Master 读取 �?保持毫秒�?�?);
  console.log('  4. IM 客户�?�?显示时应该正�?�?);
} else {
  console.log('⚠️  仍有部分数据需要修�?);
  console.log('');
