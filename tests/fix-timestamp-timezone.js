/**
 * 数据迁移脚本: 修正数据库中已存在的时间戳时区问�? *
 * 问题: 抖音API返回的时间戳是UTC+8时区的，数据库中存储的时间戳需要减�?小时
 * 影响: cache_comments 表和 cache_messages 表中的所�?createdAt 字段
 *
 * 警告: 此脚本会修改数据库！运行前请备份�? */
...
// 时区偏移�?const TIMEZONE_OFFSET = 8 * 3600; // 8小时 = 28800�?
console.log('📊 �?�? 检查需要修正的数据...\n');

// 检�?cache_comments �?const comments = db.prepare(`
  SELECT
    id,
    account_id,
...
// 检�?cache_messages �?const messages = db.prepare(`
  SELECT
    id,
    account_id,
...
readline.question('⚠️  即将修改数据库！是否继续�?输入 yes 继续): ', (answer) => {
  ...
  try {
    ...
    // 修正 cache_comments �?    console.log('正在修正 cache_comments �?..');
    ...
    console.log(`�?已更�?${commentsResult.changes} 条评论数据\n`);

    // 修正 cache_messages �?    console.log('正在修正 cache_messages �?..');
    ...
    console.log(`�?已更�?${messagesResult.changes} 条私信数据\n`);
