...
const checkAPI = () => {
  return new Promise((resolve, reject) => {
    ...
    req.on('error', (err) => {
      console.log('❌ Master HTTP API 无响应');
...
const checkWebSocket = () => {
  return new Promise((resolve) => {
    ...
    socket.on('connect', () => {
      console.log('✅ WebSocket 端口可访问 (端口 3000)');
      socket.destroy();
      resolve(true);
    });
...
(async () => {
  ...
  console.log('2️⃣ 检查 HTTP API...');
  const apiOk = await checkAPI();
  console.log('');

  console.log('3️⃣ 检查 WebSocket 端口...');
  ...
  if (dbOk && apiOk && wsOk) {
    ...
    console.log('');
    console.log('请确认：');
    console.log('   1. Master 已启动: cd packages/master && npm start');
    console.log('   2. 端口 3000 未被占用');
    console.log('   3. 数据库文件存在且有数据');
    console.log('');
    process.exit(1);
  }
})();
