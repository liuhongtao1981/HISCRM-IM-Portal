...
async function triggerCommentCrawl() {
  ...
  // 1. 连接数据库获取账户信�?  const dbPath = path.join(__dirname, '../packages/master/data/master.db');
  ...
  console.log(`   用户�? ${account.platform_username || '未设�?}`);
  console.log(`   平台用户ID: ${account.platform_user_id || '未设�?}`);
  console.log(`   Worker: ${account.worker_id || '未分�?}\n`);

  // 2. 连接�?Master �?Admin 命名空间
  const socket = io('http://localhost:3000/admin');

  socket.on('connect', () => {
    console.log('�?已连接到 Master 服务器\n');

    // 3. 发送抓取评论任�?    console.log('📤 发送评论抓取任�?..\n');
    ...
    const taskMessage = {
      ...
      payload: {
        ...
        options: {
          maxVideos: 1,           // 只抓�?个视�?          includeDiscussions: true,  // 包含讨论(回复)
        }
      }
    };
