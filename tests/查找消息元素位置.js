...
async function findMessageElements() {
  ...
  try {
    ...
    console.log(`找到 ${conversations.length} 个会话，点击最后一�?..\n`);
    await conversations[conversations.length - 1].click();
    await page.waitForTimeout(3000);

    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    console.log('📍 查找所�?[class*="message"] 元素');
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n');
    ...
    console.log(`找到 ${messageInfo.length} �?[class*="message"] 元素\n`);

    messageInfo.forEach((msg, i) => {
      console.log(`━━�?元素 ${i + 1} ━━━`);
      console.log(`<${msg.tag}> class="${msg.className}"`);
      console.log(`文本: ${msg.text}`);
      console.log(`�?React Fiber: ${msg.hasReactFiber ? '�? : '�?}`);
      console.log(`\n父级�?(�?10 �?:`);
    ...
    fiberData.forEach((data, i) => {
      ...
      console.log(`文本: ${data.text || '(无文�?'}`);

      if (data.error) {
        console.log(`�?${data.error}`);
      } else if (data.propsAtDepth.length === 0) {
        console.log(`⚠️  �?30 �?Fiber 树中未找到相�?props`);
      } else {
        console.log(`�?找到 ${data.propsAtDepth.length} 层包含相�?props:`);
