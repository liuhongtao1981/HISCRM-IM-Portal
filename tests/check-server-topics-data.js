/**
 * 检查服务端返回�?topics 数据
 * 连续请求 3 次，看看每次返回的数据是否一�? */
const io = require('socket.io-client');

const MASTER_URL = 'http://localhost:3000';

console.log('=== 检查服务端 Topics 数据一致�?===\n');
...
socket.on('monitor:channels', (data) => {
  ...
  console.log('连续请求 3 �?topics，间�?1 �?..\n');

  // 请求 3 �?  for (let i = 1; i <= 3; i++) {
    setTimeout(() => {
      console.log(`\n[请求 ${i}] 发�?monitor:request_topics`);
...
function analyzeResponses() {
  ...
  allResponses.forEach(resp => {
    console.log(`响应 ${resp.index}: 评论=${resp.commentUnread}, 私信=${resp.privateUnread}, 总计=${resp.totalUnread}, Topics�?${resp.topicsCount}`);
  });

  // 检查一致�?  const uniqueComment = [...new Set(allResponses.map(r => r.commentUnread))];
  const uniquePrivate = [...new Set(allResponses.map(r => r.privateUnread))];
  const uniqueTotal = [...new Set(allResponses.map(r => r.totalUnread))];

  console.log(`\n一致性检查：`);
  console.log(`评论未读数的唯一�? ${JSON.stringify(uniqueComment)} ${uniqueComment.length === 1 ? '�?一�? : '�?不一�?}`);
  console.log(`私信未读数的唯一�? ${JSON.stringify(uniquePrivate)} ${uniquePrivate.length === 1 ? '�?一�? : '�?不一�?}`);
  console.log(`总未读数的唯一�? ${JSON.stringify(uniqueTotal)} ${uniqueTotal.length === 1 ? '�?一�? : '�?不一�?}`);
  ...
  if (uniqueComment.length > 1 || uniquePrivate.length > 1) {
    ...
    first.detailedTopics.forEach(t1 => {
      ...
      if (t2) {
        if (t1.unreadCount !== t2.unreadCount) {
          console.log(`  ⚠️  "${t1.title}" 未读数变�? ${t1.unreadCount} �?${t2.unreadCount}`);
        }
      } else {
        console.log(`  ⚠️  �?2 次响应缺�?Topic: "${t1.title}"`);
      }
    });
    ...
    second.detailedTopics.forEach(t2 => {
      ...
      if (!t1) {
        console.log(`  ⚠️  �?1 次响应缺�?Topic: "${t2.title}"`);
      }
    });
  } else {
    console.log('\n�?服务端数据完全一致，问题在客户端�?);
