/**
 * 检查 Master WebSocket 发送的数据结构
 * 验证 topics 和 messages 是否包含正确的字段
 */
...
socket.on('monitor:channels', (data) => {
  console.log('📡 收到 channels:', data.channels.length, '个');
...
socket.on('monitor:topics', (data) => {
  ...
  if (receivedTopics.length > 0) {
    ...
    if (privateTopics.length > 0) {
      console.log('═'.repeat(80));
      console.log('📋 第一个私信主题 (isPrivate=true) 的完整结构:');
      console.log('═'.repeat(80));
...
socket.on('monitor:messages', (data) => {
  console.log('');
  console.log('📡 收到 messages:', data.messages.length, '条');
  ...
  if (receivedMessages.length > 0) {
    ...
    // 统计有 messageCategory 字段的消息
    const privateMessages = receivedMessages.filter(m => m.messageCategory === 'private');
    ...
    console.log(`✅ 总消息: ${receivedMessages.length}`);
    ...
    if (privateMessages.length > 0) {
      console.log('═'.repeat(80));
      console.log('📋 第一条私信消息 (messageCategory=private) 的完整结构:');
      console.log('═'.repeat(80));
