/**
 * Chrome 启动诊断脚本
 * 测试不同的浏览器启动方式以确�?exitCode=21 的根本原�?
 */
...
async function diagnoseChrome() {
  ...
  console.log('目标: 确定为什�?Chrome �?exitCode=21 退出\n');

  // 测试 1: Playwright 捆绑�?Chromium (基础启动)
  try {
    ...
    console.log(`   浏览器版�? ${browser.version()}`);
    await browser.close();
    console.log('   �?浏览器正常关闭\n');
  } catch (error) {
  ...
  try {
    ...
    console.log(`   浏览器版�? ${context.browser().version()}`);
    await context.close();
    console.log('   �?上下文正常关闭\n');
  } catch (error) {
  ...
  // 测试 5: 检�?Playwright 安装
  console.log('📋 测试 5: Playwright 安装信息...');
  try {
    const executablePath = chromium.executablePath();
    console.log(`   Chromium 可执行文�? ${executablePath}`);
  ...
  console.log('- 如果测试 3 失败但测�?1 成功 �?持久化上下文问题，改�?launch + newContext');
  console.log('- 如果测试 4 成功 �?使用持久化上下文 + 系统 Chrome');
  console.log('- 如果所有测试都失败 �?系统环境问题 (防病�?安全策略)');
}
