 ...
 *
 * 问题: 抖音创作者中心显�?"10�?0�?04:28"
 *       但IM客户端显�?"10/30 12:28"
 *       差了8小时
 *
 * 假设: 抖音API返回的create_time时间戳是UTC+8时区�? *       需要减�?小时才是真正的UTC时间�? */
...
console.log(`找到 ${comments.length} �?0�?0日左右的评论\n`);
...
comments.forEach((comment, index) => {
  ...
  if (new Date(correctedTimestamp * 1000).toLocaleString('zh-CN', { hour: '2-digit', minute: '2-digit' }) === '04:28') {
    console.log('   �?假设2正确！抖音API返回的是UTC+8时间�?);
  } else {
    console.log('   �?需要进一步分�?);
  }
...
console.log('2. 我们需要在存储时减�?小时�?8800秒）来转换为标准UTC时间�?);
