...
const TEST_CONFIG = {
  ...
  monitorInterval: 5000,  // �?秒输出一次状�?
  testDuration: 60000,    // 测试持续60�?
};
...
function printDataManagerStats() {
  ...
  console.log(`\n数据集合:`);
  console.log(`  �?会话: ${stats.collections.conversations.total} �?(待同�? ${stats.collections.conversations.dirty})`);
  console.log(`  �?消息: ${stats.collections.messages.total} �?(待同�? ${stats.collections.messages.dirty})`);
  console.log(`  �?作品: ${stats.collections.contents.total} �?(待同�? ${stats.collections.contents.dirty})`);
  console.log(`  �?评论: ${stats.collections.comments.total} �?(待同�? ${stats.collections.comments.dirty})`);
  console.log(`  �?通知: ${stats.collections.notifications.total} �?(待同�? ${stats.collections.notifications.dirty})`);
...
function validateDataRelations() {
  ...
  if (allContents.length > 0) {
    ...
    allContents.slice(0, 3).forEach(content => {
      ...
      const title = content.title ? content.title.substring(0, 20) : '无标�?;
  ...
  if (allComments.length > 0) {
    ...
    if (replies.length > 0) {
      console.log(`\n  🔗 评论 �?回复:`);
      console.log(`     一级评�? ${topLevelComments.length} 条`);
      ...
      if (invalidReplies.length > 0) {
        console.log(`     �?发现 ${invalidReplies.length} 条无效回�?(父评论不存在)`);
        allValid = false;
      } else {
...
function startMonitoring() {
  ...
  monitorTimer = setInterval(() => {
    ...
    if (Date.now() - startTime >= TEST_CONFIG.testDuration) {
      console.log(`\n⏱️  测试时间�?(${TEST_CONFIG.testDuration / 1000}�?，停止监控`);
      stopMonitoring();
      process.exit(0);
    }
...
async function runTest() {
  try {
    console.log('1️⃣  初始化抖音平�?..');
    platform = new DouyinPlatform();

    console.log('2️⃣  获取 DataManager (测试懒加�?...');
    ...
    if (!accountBrowser || !accountBrowser.page) {
      console.log('⚠️  账户浏览器未运行，需要先启动 Worker 并登录账�?);
    ...
    for (const crawlerType of TEST_CONFIG.crawlerTypes) {
      try {
        ...
        switch (crawlerType) {
          ...
          default:
            console.log(`⚠️  未知的爬虫类�? ${crawlerType}`);
        }
