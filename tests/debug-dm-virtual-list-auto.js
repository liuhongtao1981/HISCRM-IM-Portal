/**
 * 调试抖音私信虚拟列表 - 自动化版�? * 自动点击会话并分析虚拟列表内�? */
  ...
  try {
    ...
    for (const selector of conversationSelectors) {
      try {
        ...
        console.log(`选择�?"${selector}" 找到 ${conversations.length} 个元素`);

        if (conversations.length > 0) {
          console.log(`\n点击第一个会话（使用选择�? ${selector}�?..`);
    ...
    if (!conversationClicked) {
      ...
      const domStructure = await page.evaluate(() => {
        ...
        return getAllClasses(body).slice(0, 50); // 只取�?0�?      });
    ...
    // ============================================================
    // 步骤 2: 详细分析消息元素�?React Fiber 结构
    // ============================================================
    console.log('\n【步�?2】分�?React Fiber 结构');
    console.log('-'.repeat(60));

    const step2Result = await page.evaluate(() => {
      // 尝试多个选择�?      const allElements = document.querySelectorAll('[class*="message"], [class*="Message"], [class*="item"], [role="listitem"]');
      ...
      Array.from(allElements).slice(0, maxSamples).forEach((element, index) => {
        ...
        // 查找所有以 __react 开头的�?        const reactKeys = Object.keys(element).filter(key => key.startsWith('__react'));
    ...
    if (step2Result.error) {
      ...
      console.log(`分析�?${step2Result.samples.length} 个元�?\n`);
      ...
      step2Result.samples.forEach(sample => {
        ...
        console.log(`  文本: ${sample.textPreview || '(无文�?'}`);
        console.log(`  React Fiber: ${sample.hasFiber ? '�?存在' : '�?不存�?}`);

        if (sample.hasFiber) {
          console.log(`  Fiber �? ${sample.fiberKey}`);
          console.log(`  Props �?(${sample.propsKeys.length}�?:`);
    ...
    if (step3Result.results.length === 0) {
      console.log('�?没有�?React Fiber 中找到消息数�?);
    } else {
      console.log(`�?�?${step3Result.results.length} 个元素中找到消息数据:\n`);
      ...
      step3Result.results.forEach(result => {
        ...
        result.findings.forEach((finding, idx) => {
          ...
          console.log(`    总Props�? ${finding.allPropsCount}`);
    ...
    console.log(`扫描元素�? ${step4Result.totalElements}`);
    console.log(`提取消息�? ${step4Result.extractedCount}`);

    if (step4Result.extractedCount === 0) {
      console.log('\n�?当前提取逻辑返回 0 条消息！');
      console.log('\n可能原因:');
      console.log('  1. Props 中没�?conversationId/serverId/content/message 属�?);
      console.log('  2. 消息数据在更深层�?Fiber 节点�?);
      console.log('  3. 抖音更新了页面结�?);
    } else {
    ...
    console.log('\n' + '='.repeat(60));
    console.log('调试完成！浏览器将保持打开 30 �?..');
    console.log('='.repeat(60) + '\n');

    // 保持浏览器打开 30 �?    await page.waitForTimeout(30000);
