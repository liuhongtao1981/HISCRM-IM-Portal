...
comments.forEach(c => {
  ...
  console.log(`  作�? ${c.author_name}`);
  console.log(`  内容: ${c.content ? c.content.substring(0, 50) : 'N/A'}${c.content && c.content.length > 50 ? '...' : ''}`);
  console.log(`  回复�? ${c.reply_count} 条`);
  console.log(`  点赞�? ${c.stats_like_count}`);
...
console.log(`实际讨论记录�? ${discussionCount.count}`);

if (stats.total_reply_count > discussionCount.count) {
  console.log(`\n⚠️  发现问题: reply_count 显示�?${stats.total_reply_count} 条回复，�?discussions 表只�?${discussionCount.count} 条记录`);
