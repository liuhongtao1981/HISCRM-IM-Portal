/**
 * 验证数据库中的 rawData.secSender 字段是否存在
 *
 * 目的：检查是否可以通过 secSender 匹配会话的 user_id
 */
...
console.log('统计结果:');
console.log('='.repeat(80));
console.log(`  有 secSender: ${hasSecSender} 条 (${(hasSecSender / messages.length * 100).toFixed(1)}%)`);
console.log(`  无 secSender: ${noSecSender} 条 (${(noSecSender / messages.length * 100).toFixed(1)}%)`);
console.log('');

if (hasSecSender > 0) {
  console.log('='.repeat(80));
  console.log('前5条有 secSender 的消息:');
  console.log('='.repeat(80));
  ...
  sampleMessages.forEach(msg => {
    ...
    if (match) {
      ...
      console.log(`  ✅ 匹配到会话: ${convData.userName || '(无名称)'}`);
      matchCount++;
    } else {
  ...
  if (matchCount > 0) {
    ...
    console.log('   2. 在查询逻辑中添加 rawData.secSender 匹配');
    console.log('   3. 重启 CRM-IM Server');
    console.log('   4. 测试客户端是否显示私信');
  } else {
    console.log('⚠️ secSender 存在，但无法匹配到会话');
    console.log('可能原因：secSender 对应的会话不在数据库中');
  }

} else {
  console.log('='.repeat(80));
  console.log('⚠️ 未发现 secSender 字段');
  console.log('='.repeat(80));
  console.log('');
  console.log('📝 下一步行动:');
  console.log('   1. 修改 Worker 爬虫 (crawl-direct-messages-v2.js)');
  console.log('   2. 增加 secSender 字段提取逻辑');
  console.log('   3. 重启 Worker，重新爬取数据');
