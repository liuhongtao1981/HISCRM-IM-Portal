...
async function testPlatformInitialization() {
  console.log('========================================');
  console.log('测试平台初始化流�?);
  console.log('========================================\n');

  try {
    // 1. 加载平台管理�?    const PlatformManager = require('../packages/worker/src/platform-manager');

    // 创建模拟�?workerBridge �?browserManager
    ...
    // 创建平台管理器实�?    const platformManager = new PlatformManager(mockWorkerBridge, mockBrowserManager);

    // 2. 加载平台
    console.log('1. 加载平台...');
    await platformManager.loadPlatforms();
    console.log(`   �?已加�?${platformManager.platforms.size} 个平台\n`);
    ...
    // 4. 检�?initialize 方法
    console.log('3. 检�?initialize 方法...');
    console.log('   douyinPlatform.initialize 存在:', typeof douyinPlatform.initialize);
    console.log('   douyinPlatform.initialize 是函�?', typeof douyinPlatform.initialize === 'function');

    if (typeof douyinPlatform.initialize !== 'function') {
      console.error('   �?initialize 不是一个函�?);
      return;
    }
