...
let requestCount = 0;
const topicsHistory = [];  // 记录每次请求�?topics
...
socket.on('monitor:channels', (data) => {
  ...
  // 选择第一个频道进行测�?  testChannelId = channels[0].id;
  console.log(`测试频道: ${channels[0].name} (${testChannelId})\n`);
  console.log('开始连续请�?topics 10 次，每次间隔 500ms...\n');

  // 连续请求 10 �?  for (let i = 1; i <= 10; i++) {
    setTimeout(() => {
      requestCount = i;
...
socket.on('monitor:topics', (data) => {
  ...
  console.log(`[响应] 收到 topics (请求 #${topicsHistory.length + 1}): ${topics.length} �?topics`);

  // 计算未读�?  let totalUnread = 0;
  let privateUnread = 0;
  let commentUnread = 0;
  ...
  console.log(`  - 总未�? ${totalUnread}`);

  // 保存到历史记�?  topicsHistory.push({
    receivedAt,
    topicsCount: topics.length,
    totalUnread,
    privateUnread,
    commentUnread,
    topics: JSON.parse(JSON.stringify(topics))  // 深拷�?  });
});
...
function analyzeResults() {
  ...
  console.log(`\n评论未读数的唯一�? ${JSON.stringify(uniqueCommentCounts)}`);
  console.log(`私信未读数的唯一�? ${JSON.stringify(uniquePrivateCounts)}`);
  ...
  if (uniqueTopicsCounts.length > 1) {
    console.log(`\n⚠️  Topics 数量在变化！�?${uniqueTopicsCounts.length} 个不同的值`);
    console.log(`值为: ${uniqueTopicsCounts.join(', ')}`);
  } else {
    console.log(`\n�?Topics 数量稳定 (${uniqueTopicsCounts[0]} �?`);
  }
    ...
    first.topics.forEach((topic1, index) => {
      ...
      if (topic2) {
        if (topic1.unreadCount !== topic2.unreadCount) {
          console.log(`  ⚠️  Topic "${topic1.title}" 未读数变�? ${topic1.unreadCount} �?${topic2.unreadCount}`);
        }
