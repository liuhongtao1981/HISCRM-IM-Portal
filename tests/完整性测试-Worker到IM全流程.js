/**
 * 完整性测�?- Worker �?Master �?IM API 全流�? *
 * 测试范围�? * 1. Master 服务器状态检�? * 2. Worker 连接状态检�? * 3. DataStore 数据检�? * 4. IM API 接口测试�? 个接口）
 * 5. 数据一致性验�? */

const http = require('http');

console.log('==================================================');
console.log('HisCRM-IM 完整性测�?);
...
// 测试 1: Master 服务器状�?async function testMasterStatus() {
  testsTotal++;
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  console.log('测试 1/5: Master 服务器状态检�?);
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n');
  ...
  try {
    ...
    if (statusCode === 200 && data.success) {
      ...
      testResults.master.message = 'Master 服务器运行正�?;

      console.log('�?Master 服务�? 运行�?);
      console.log('   端口:', data.data.port || 3000);
      console.log('   环境:', data.data.environment || 'development');
      console.log('   运行时间:', Math.floor((data.data.uptime || 0) / 1000) + '�?);

      // 存储 Worker 信息
      if (data.data.workers) {
        testResults.workers.count = data.data.workers.length;
        console.log('   已连�?Worker:', data.data.workers.length + '�?);
      }

      // 存储 DataStore 信息
      if (data.data.dataStore) {
        testResults.dataStore.data = data.data.dataStore;
        console.log('   DataStore 账户�?', data.data.dataStore.totalAccounts || 0);
      }
    } else {
      testsFailed++;
    ...
    console.log('   提示: 请运�?"npm run start:master" 启动服务�?);
  }

  console.log();
}

// 测试 2: Worker 连接状�?async function testWorkerStatus() {
  testsTotal++;
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  console.log('测试 2/5: Worker 连接状态检�?);
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n');
  ...
  if (testResults.workers.count === 0) {
    ...
    console.log('�?Worker 连接: 0 �?);
    console.log('   提示: 请运�?"npm run start:worker" 启动 Worker');
  } else {
    testsPassed++;
    testResults.workers.status = 'passed';
    testResults.workers.message = `${testResults.workers.count} �?Worker 已连接`;
...
// 测试 3: DataStore 数据检�?async function testDataStore() {
  testsTotal++;
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  console.log('测试 3/5: DataStore 数据检�?);
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n');
  ...
  console.log('  总通知�?', ds.totalNotifications || 0);
  ...
  if (hasData) {
    ...
    testResults.dataStore.message = 'DataStore 有数�?;
    console.log('\n�?DataStore: 有数�?);
  } else {
    testsFailed++;
    ...
    console.log('\n⚠️  DataStore: 暂无数据');
    console.log('   说明: Worker 正在爬取数据，或账户未登�?);
...
async function testIMApis() {
  ...
  for (const api of apis) {
    try {
      ...
      if (statusCode === 200 && data.status_code === 0) {
        ...
        console.log(`�?${api.name} API: 正常 (返回 ${items.length} 条数�?`);
      } else {
        apisFailed++;
        testResults.imApis.results.push({
          name: api.name,
  ...
  if (apisFailed === 0) {
    ...
    testResults.imApis.message = '所�?API 正常';
  } else {
    testsFailed++;
    testResults.imApis.status = 'failed';
    testResults.imApis.message = `${apisFailed} �?API 失败`;
  }

  console.log();
}

// 测试 5: 数据一致性验�?async function testConsistency() {
  testsTotal++;
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  console.log('测试 5/5: 数据一致性验�?);
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n');

  // 验证 DataStore �?IM API 的数据是否一�?  const dsConversations = testResults.dataStore.data.totalConversations || 0;
  ...
  // �?IM API 获取的数�?  const apiResults = testResults.imApis.results;
  ...
  console.log('数据对比:');
  console.log('  会话�?- DataStore:', dsConversations, '| IM API:', apiConversations);
  console.log('  私信�?- DataStore:', dsMessages, '| IM API:', apiMessages);
  console.log('  评论�?- DataStore:', dsComments, '| IM API:', apiComments);
  console.log('  作品�?- DataStore:', dsContents, '| IM API:', apiContents);
  ...
  if (consistent) {
    ...
    testResults.consistency.message = '数据一�?;
    console.log('\n�?数据一致�? 通过');
  } else {
    testsFailed++;
    testResults.consistency.status = 'failed';
    testResults.consistency.message = '数据不一�?;
    console.log('\n�?数据一致�? 失败');
    console.log('   DataStore 有数据但 API 未返�?);
  }
...
// 打印最终报�?function printFinalReport() {
  ...
  console.log(`  成功�? ${Math.round(testsPassed / testsTotal * 100)}%`);

  console.log('\n📋 详细结果:');
  console.log(`  1. Master 服务�? ${testResults.master.status === 'passed' ? '�? : '�?} ${testResults.master.message}`);
  ...
  console.log(`  5. 数据一致�? ${testResults.consistency.status === 'passed' ? '�? : '�?} ${testResults.consistency.message}`);

  console.log('\n💡 建议:');

  if (testResults.master.status !== 'passed') {
    console.log('  - 启动 Master 服务�? npm run start:master');
  }
  ...
  if (testResults.dataStore.status === 'warning') {
    console.log('  - 等待 30-60 秒让 Worker 推送数�?);
    console.log('  - 确保账户已登�? 使用 Admin Web 登录账户');
    console.log('  - 检查账户状�? node tests/查询账户状�?js');
  }

  if (testResults.consistency.status !== 'passed') {
    console.log('  - 检�?DataStore 日志');
    console.log('  - 重启 Master �?Worker');
  }
  ...
  if (testsPassed === testsTotal) {
    ...
    console.log('  �?系统基本正常，部分功能需要调�?);
  } else if (testsPassed >= testsTotal * 0.5) {
    console.log('  ⚠️  系统部分正常，需要处理多个问�?);
  } else {
    console.log('  �?系统存在重大问题，需要立即处�?);
  }
...
// 主测试流�?async function runTests() {
  try {
    ...
    // 退出码�?=全部通过�?=有失�?    process.exit(testsFailed === 0 ? 0 : 1);
  } catch (err) {
    console.error('\n�?测试执行失败:', err);
    process.exit(1);
  }
