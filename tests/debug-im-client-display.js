 ...
 * IM 客户端显示问题调试脚本
 *
 * 用途: 连接到 Master，获取并分析私信数据结构
 * 验证: topics 的 isPrivate 字段和 messages 的 messageCategory 字段
 */
...
console.log('🔍 IM 客户端显示问题调试');
console.log('='.repeat(80));
console.log('');
console.log(`连接到 Master: ${MASTER_URL}`);
console.log('');
...
socket.on('monitor:channels', (data) => {
  ...
  if (channels.length === 0) {
    console.log('❌ 没有频道数据，请确认 Worker 已爬取数据');
    socket.disconnect();
    process.exit(1);
  }
...
socket.on('monitor:topics', (data) => {
  ...
  console.log(`   作品主题 (无 isPrivate):   ${contentTopics.length}`);
  console.log('');
  ...
  if (privateTopics.length === 0) {
    ...
    console.log('');
    console.log('原因分析:');
    console.log('  1. DataStore 中没有 conversations 数据');
    console.log('  2. Master 代码未正确设置 isPrivate 字段');
    console.log('  3. Worker 未爬取私信数据');
    console.log('');
    console.log('建议:');
    console.log('  检查 packages/master/src/communication/im-websocket-server.js:457');
    ...
    // 打印第一个私信主题
    const firstPrivate = privateTopics[0];
...
socket.on('monitor:messages', (data) => {
  ...
  console.log('');
  console.log('='.repeat(80));
  console.log(`📡 收到 messages: ${messages.length} 条 (topicId: ${topicId.substring(0, 20)}...)`);
  console.log('='.repeat(80));
  console.log('');
  ...
  console.log(`   无 messageCategory 字段:     ${unknownMessages.length}`);
  console.log('');

  if (privateMessages.length === 0 && topicId === privateTopicId) {
    console.log('⚠️  关键问题: 私信主题的消息没有 messageCategory="private"');
    console.log('');
    console.log('原因分析:');
    console.log('  Master 代码未正确设置 messageCategory 字段');
    console.log('');
    console.log('建议:');
    console.log('  检查 packages/master/src/communication/im-websocket-server.js:590');
    ...
    // 打印第一条私信消息
    const firstMessage = privateMessages[0] || messages[0];
    console.log('📋 第一条消息:');
  ...
  if (hasPrivateTopics && hasPrivateMessages) {
    ...
    console.log('   1. WebSocket 事件未正确监听');
    console.log('   2. Redux store 未更新');
    console.log('   3. 过滤逻辑有问题');
    console.log('   4. 消息与主题关联错误');
    console.log('');
    console.log('下一步:');
    console.log('   1. 启动 IM PC 客户端: cd packages/crm-pc-im && npm run dev');
    console.log('   2. 打开浏览器 http://localhost:5173');
    console.log('   3. 按 F12 打开开发者工具');
    console.log('   4. 查看 Console 日志，确认是否收到 WebSocket 事件');
    ...
    console.log('   2. 找到第 447 行附近的会话主题创建代码');
    ...
    console.log('修复步骤:');
    console.log('   1. 编辑 packages/master/src/communication/im-websocket-server.js');
    console.log('   2. 找到第 582 行附近的私信消息创建代码');
