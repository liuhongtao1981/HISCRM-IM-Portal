...
try {
  ...
  for (const table of TABLES_TO_CLEAR) {
    ...
    if (beforeCount > 0) {
      ...
      console.log(`  �?${table.padEnd(20)} 已清�?${beforeCount} 条记录`);
  ...
  for (const table of TABLES_TO_CLEAR) {
    ...
    if (result.count === 0) {
      ...
      console.log(`  �?${table.padEnd(20)} ${result.count.toString().padStart(8)} �?(清空失败!)`);
    }
  }
  ...
  for (const table of SYSTEM_TABLES) {
    try {
      const result = db.prepare(`SELECT COUNT(*) as count FROM ${table}`).get();
      console.log(`  �?${table.padEnd(20)} ${result.count.toString().padStart(8)} �?(保留)`);
    } catch (error) {
