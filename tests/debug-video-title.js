...
async function debugVideoTitle() {
  console.log('╔══════════════════════════════════════════════════════════════╗');
  console.log('║         调试：抖音评论 API 响应中的视频标题信息              ║');
  console.log('╚══════════════════════════════════════════════════════════════╝\n');

  console.log('⚠️  此脚本需要在 Worker 环境中运行');
  console.log('   请执行以下步骤：');
  console.log('   1. 启动 Master: npm run start:master');
  console.log('   2. 启动 Worker: npm run start:worker');
  console.log('   3. 等待 Worker 登录账户并抓取评论');
  ...
  console.log('   此匹配方式问题：');
  console.log('   ❌ 两个视频评论数相同时会匹配错误');
  console.log('   ❌ videosToClick 数组与 API 响应的 item_id 无直接关联\n');

  console.log('💡 解决方案：');
  console.log('   1. 查看 API 响应中是否包含视频标题信息');
  console.log('   2. 如果有，直接从 API 响应中提取');
  console.log('   3. 如果没有，需要建立 DOM 元素索引与 item_id 的映射\n');
