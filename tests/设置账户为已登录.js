...
async function setAccountLoggedIn() {
  ...
  try {
    ...
    console.table({
      ...
      '用户�?: updatedAccount.username || 'N/A',
      '状�?: updatedAccount.status,
      '激�?: updatedAccount.is_active ? '�? : '�?
    });

    console.log('\n�?账户状态已更新!');
    console.log('📌 注意: 这只是为了测试目的，实际使用中账户需要真实登�?);
