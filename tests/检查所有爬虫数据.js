...
console.log(`  🗣�? 讨论 (discussions): ${discussionsCount} 条`);
...
if (messagesCount > 0) {
  ...
  recentMessages.forEach((m, i) => {
    ...
    console.log(`     检测时�? ${new Date(m.detected_at * 1000).toLocaleString('zh-CN')}`);
...
// 4. 检查作品表结构
console.log('📋 检�?contents 表是否存�?');
try {
  const tableInfo = db.prepare('PRAGMA table_info(contents)').all();
  if (tableInfo.length > 0) {
    console.log('  �?contents 表存�?);
    console.log(`  字段�? ${tableInfo.length}`);
  } else {
...
if (worksCount === 0 && commentsCount === 0 && discussionsCount === 0) {
  console.log('  �?作品、评论、讨论数据均未入�?);
  console.log('  �?私信数据已入�?(' + messagesCount + ' �?');
  console.log('');
  console.log('  可能原因:');
  console.log('  1. Worker 只配置了私信爬虫任务');
  console.log('  2. 作品/评论/讨论爬虫未自动触�?);
  console.log('  3. 表结构不存在导致入库失败');
} else {
  console.log('  �?所有爬虫数据均已入�?);
}
console.log('========================================\n');
