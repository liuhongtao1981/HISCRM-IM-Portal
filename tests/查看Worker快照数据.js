/**
 * 查看 Worker 的快照数�? * 直接连接�?Worker 命名空间，获�?DataStore 快照，分析私信的会话 ID 映射
 */
...
// 连接�?Worker 命名空间（模�?Master 请求 Worker 数据�?const socket = io(`${MASTER_URL}/worker`, {
...
socket.on('connect', () => {
  ...
  // 请求 Worker 同步数据（这会触�?Worker 发送快照）
  console.log('\n发�?master:request_sync 请求...');
...
socket.on('worker:data_sync', (data) => {
  ...
  if (problemCount > 0) {
    ...
    console.log('这说�?Worker 爬取的数据本身就是错的，或�?mapMessageData() 没有正确处理�?);
  } else {
    console.log('\n�?Worker 快照数据中的会话 ID 映射正确�?);
    console.log('问题可能�?Master 的转换层�?IM WebSocket Server�?);
  }
...
// 30 秒超�?setTimeout(() => {
  ...
  console.log('\n可能的原�?');
  console.log('1. Worker 没有连接�?Master');
